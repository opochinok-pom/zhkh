...
│   └── vite.config.js
├── supabase/
│   └── schema.sql         # CREATE TABLE payments, history, fitness_*
...
Второе приложение в этом же репозитории (кнопка **🏋️ Фитнес** в шапке ЖКХ, или `/?app=fitness`). Программа тренировок на 5 дней (сила пн/ср/пт, кардио вт/чт), которая **подстраивается под факт тренировок и сон**.
